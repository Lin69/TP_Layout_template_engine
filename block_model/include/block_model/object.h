...
struct String {
  ...
  [[nodiscard]] bool is_empty() const { return str.empty(); };
  String& operator+(const String& src) {
    this->str = this->str + src.str;
    return *this;
  };
  bool operator==(const String& src) { return this->str == src.str; }
  std::string str;
};
...
struct Vector {
  ...
  [[nodiscard]] bool is_empty() const { return vec.empty(); };
  std::vector<T> vec;
};
...
struct Map {
  ...
  void insert(const String& value) { content.push_back(value); }

  void set_tag(const String& value) { tag = value; };

  void set_end(bool value = true) { end_tag = value; };
...
class Object {
 ...
 protected:
  ...
  int id_attr{};  // При этом при уничтожении объекта последующие id не
                  // уменьшаются
